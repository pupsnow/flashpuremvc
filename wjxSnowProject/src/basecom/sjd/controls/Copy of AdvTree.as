package  basecom.sjd.controls
{
	...
	import basecom.sjd.utils.XMLUtils;
	import flash.events.Event;
	import mx.core.ClassFactory;
	import basecom.sjd.controls.treeClasses.AdvTreeItemRenderer;
	...
	public class AdvTree extends Tree{
		...
		public function AdvTree(){
			super();
			itemRenderer = new ClassFactory(AdvTreeItemRenderer);
			this.addEventListener(FlexEvent.CREATION_COMPLETE, loaded);
		}
...
//////////////////////////////////////////////////////////////////////////////////////	
		...
		override public function showDropFeedback(event:DragEvent):void{
			...
			if(_dropData){
	    		event.localY = event.localY + _dropData.rowHeight;
	    	}
			...
			if (_dropData.parent)
			{
				depth = getParentDepth(_dropData.parent);
			...
			event.localY -= _dropData.rowHeight / 2;
			...
			if(cNode && dataDescriptor.isBranch(cNode) &&
				( (yOff >= 0 && yOff < _dropData.rowHeight / 4 * 1) || ( yOff > _dropData.rowHeight / 4 * 3 && yOff <= _dropData.rowHeight / 4 * 4) )
				){
	    ...
	    override public function calculateDropIndex(event:DragEvent = null):int{
			if (event){
				if(_dropData){
					event.localY = event.localY + _dropData.rowHeight / 2;
			            ...
			            for (var i:int = 0; i < items.length; i++){ 
							...
							if(event.action == DragManager.MOVE){
	 		                ...
	 		                if (parent == _dropData.parent && index < _dropData.index){
	  		                	if(event.action == DragManager.MOVE){
	  		                		addChildItem(_dropData.parent, (items[i] as XML).copy(), _dropData.index - 1);
	  		                	}else{
	  		                		addChildItem(_dropData.parent, (items[i] as XML).copy(), _dropData.index);
	  		                	}
	  		                }else{
	   			            	if(event.action == DragManager.MOVE){
	   			            		addChildItem(_dropData.parent, (items[i] as XML).copy(), _dropData.index);
			...
			if ((event.action == DragManager.MOVE || event.action == DragManager.COPY) && dragMoveEnabled){
				...
				for (var i:int = 0; i < items.length; i++){
					...
					if(items[i] is XML){
				 	...
				 	if(event.action == DragManager.MOVE){
   	            		addChildItem(_dropData.parent, itemXML, _dropData.index);
   	            	}else{
   	            		addChildItem(_dropData.parent, itemXML, _dropData.index);
   	            	}
				}
...
//////////////////////////////////////////////////////////////////////////////////////		
	    ...
	    protected var _dropData:Object;
	    
	    sjd_internal var yOff:Number = 0;
	    
	    sjd_internal var oldRowNum:Number = 0;
...
if(_dropData){
	pt = globalToLocal(new Point(event.stageX, event.stageY + _dropData.rowHeight / 2));
...
//////////////////////////////////////////////////////////////////////////////////////		
		...
		private function calculateMaxHorizontalScroll():void{
			...
			for each(var obj:AdvTreeItemRenderer in openList){
